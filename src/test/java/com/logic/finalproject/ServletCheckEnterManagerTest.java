...
class ServletCheckEnterManagerTest {
    ...
    void doPost() throws ServletException, IOException {
        ServletCheckEnterManager servlet = new ServletCheckEnterManager();
        ...
        String adressRedirect = "/mymanagers";
        when(request.getParameter("password")).thenReturn(adressRedirect);
        when(request.getSession()).thenReturn(session);
      //  when(statement.executeQuery("SELECT * FROM managers WHERE login = '" + login + "';")).thenReturn(resultSet);
